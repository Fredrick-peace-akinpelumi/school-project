...
const UploadAlbum = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post('http://localhost:5000/api/songs/album/', { ...songData, artist:first && first.username, track:trackArr })
  ...
  const addTrack = async (e) => {
    e.preventDefault();
    if(trackArr.length===15 || trackArr.length>10){
  ...
  return (
    <>
      ...
      <div className="container col-lg-6">
        <div className="row">
          <h3 className='text-white text-center'>Upload your Album</h3>
          <form action="" onSubmit={handleSubmit}>
            <div className="form-floating col-4 mx-auto">
              <input type="text" placeholder='album Title'
                onChange={e => { setsongData({ ...songData, albumTitle: e.target.value }) }}
                value={songData.albumTitle}
                className='form-control mb-3' />
              <label htmlFor="Music Title" className='ms-3'>album Title</label>
            </div>
            ...
            <div className='shadow p-4'>
              ...
              <p className='text-white'>Added {length} of 15</p>
            ...
            <button disabled={Loading} className='btn btn-success rounded-pill mt-4' type='submit'>Upload album</button>
